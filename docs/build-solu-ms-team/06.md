...
然后，MSTeam将确保从任务模块标题到 HTML 的键盘导航正常工作，反之亦然。
...
*   **< APP_ID > :** 这是自定义MSTeam应用的 ID。应用 ID，一个 GUID，可以在应用的 **manifest.json** 文件中找到。

*   **< TASKINFO。*>:**URL 查询字符串中的附加属性映射到 **taskInfo** 对象上的特定属性。

深层链接可以在任何地方使用，包括在MSTeam渠道或外部应用中的对话。
...
使用适配卡和机器人框架卡上的按钮，可以从MSTeam机器人调用任务模块。在练习 5 中，您将学习如何在MSTeam中使用带有机器人的任务模块。
...
*   通过运行以下命令运行MSTeam的 Yeoman 生成器: **yo teams** (参见图 [6-2](#Fig2) )。
...
使用应用栏导航菜单，选择更多添加的应用按钮。然后选择更多应用，接着上传自定义应用，然后为我或我的团队上传(图 [6-6](#Fig6) )。
...
在出现的文件对话框中，选择项目中的 Microsoft Teams 包。这个应用包是一个 ZIP 文件，可以在项目**中找到。/包**文件夹(图 [6-7](#Fig7) )。
...
安装后，您将能够在应用列表中看到该应用(图 [6-8](#Fig8) )。
...
*   在**连接通道**面板上，选择**MSTeam通道**，然后选择**保存**确认动作(图 [6-30](#Fig30) )。
...
*   通过运行以下命令为MSTeam运行 Yeoman Generator:**yo Teams**(图 [6-34](#Fig34) )。
...
使用应用栏导航菜单，选择更多添加的应用按钮。然后选择更多应用，接着上传自定义应用，然后为我或我的团队上传(图 [6-39](#Fig39) )。
...
在出现的文件对话框中，选择项目中的 Microsoft Teams 包。这个应用包是一个 ZIP 文件，可以在项目**中找到。/包**文件夹(图 [6-40](#Fig40) )。
...
安装后，您将能够在应用列表中看到该应用(图 [6-41](#Fig41) )。
...
一旦软件包上传，MSTeam将显示应用的摘要(图 [6-42](#Fig42) )。
...
您还学习了如何使用 Yeoman generator for Microsoft Teams 创建任务模块。在接下来的章节中，你将开始学习MSTeam应用的不同选项和特性。