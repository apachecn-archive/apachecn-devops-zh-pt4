...
*   通过运行以下命令运行 MSTeam 的 Yeoman 生成器: **yo teams** (参见图 [6-2](#Fig2) )。
...
*   在**连接通道**面板上，选择**MSTeam 通道**，然后选择**保存**确认动作(图 [6-30](#Fig30) )。
...
*   通过运行以下命令为 MSTeam 运行 Yeoman Generator:**yo Teams**(图 [6-34](#Fig34) )。
...
一旦软件包上传，MSTeam 将显示应用的摘要(图 [6-42](#Fig42) )。
