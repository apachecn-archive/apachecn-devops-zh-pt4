...
此外，MSTeam 开发平台支持六种不同的 ide 来开发 MSTeam 应用(图 [2-2](#Fig2) )。
...
为了让机器人与 MSTeam 交互，你必须启用如图 [2-15](#Fig15) 所示的团队通道。
...
一旦此过程完成，您应该会在您启用的频道中看到列出的网络聊天和 MSTeam(图 [2-17](#Fig17) )。
...
*   通过运行以下命令来运行 MSTeam 的 Yeoman 生成器: **yo teams** *。*见图 [2-21](#Fig21) 。
