...
*   在**连接通道**面板上，选择**MSTeam 通道**，然后选择**保存**确认动作(图 [4-13](#Fig13) )。

*   同意服务条款

一旦此过程完成，您应该会在您启用的频道中看到列出的网络聊天和 MSTeam(图 [4-14](#Fig14) )。
...
*   通过运行以下命令来运行 MSTeam 的 Yeoman 生成器: **yo teams** (图 [4-17](#Fig17) )。
...
一旦软件包上传，MSTeam 将显示应用的摘要(图 [4-25](#Fig25) )。
...
*   在搜索框中输入 ISBN 号，等待几秒钟。MSTeam 将执行搜索并返回结果(图 [4-30](#Fig30) )。
