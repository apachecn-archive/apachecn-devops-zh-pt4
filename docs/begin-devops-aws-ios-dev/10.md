...
清单 [10-1](#PC1) 显示了一个 Jenkinsfile，它定义了运行 Fastlane 测试的单级管道。
...
清单 [10-4](#PC4) 显示了将 Fastlane 构建阶段附加到现有管道 Jenkinsfile 的代码片段。
...
验证 Fastlane 是否安装在 macOS 构建代理上，如清单 [10-6](#Par13648) 所示。
...
在这里，我将建立一个新的 Jenkins freestyle 项目，如图 [10-27](#Fig27) 所示，并将其配置为运行 Fastlane 测试。
...
Fastlane 测试的构建脚本如清单 [10-10](#PC10) 所示。
...
在新增加的阶段，增加一个新的 Jenkins 测试动作，如图 [10-34](#Fig34) 所示。
...
图 [10-36](#Fig36) 显示了 Fastlane 测试的成功管道执行。
...
这个项目的构建触发器将像之前为 Fastlane 测试创建的项目一样进行配置，如图 [10-30](#Fig30) 所示。
...
图 [10-47](#Fig47) 显示了 Fastlane 测试和 AWS 设备群测试的成功流水线执行。
