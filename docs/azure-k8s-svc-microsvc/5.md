...
这一层主要关注监控微服务应用和识别应用故障，以及请求率、响应时间、遇到的任何异常等信息。对于在 AKS 上运行的应用的完整监控，您可以使用应用洞察，如图 [5-13](#Fig13) 所示。
...
接下来，您需要在您的 IDE 中启用应用洞察。我最喜欢的 IDE 是 Visual Studio，但是你也可以使用其他 IDE，比如可视化代码。使用 ide 启用 Application Insights 服务器端遥测后，选择 nu get packages➤Microsoft . application insights . aspnetcore 下载 SDK 的最新稳定版本。参见图 [15-16](#Fig16) 。
