...
服务目录是 Kubernetes 中向其他托管服务敞开大门的扩展点。它支持在集群中运行的应用使用由云提供商运营的外部软件。假设您有一个 Kubernetes 集群和一个需要消息队列的应用。您基本上有两种选择:将消息队列部署到您的集群中，或者使用您的云提供商提供的消息队列服务。第一种选择非常灵活，但它带来了操作负担。对于后一种选择，您需要在 Kubernetes 集群和云提供商的消息队列服务之间找到一个*桥*。网桥是服务目录，具有图 [7-1](#Fig1) 中的以下架构。
