...
`--ours`标志表示当前分支。这就是菜谱里的`en`。`--theirs`标志表示传递给`$ git merge`命令的分支。在这个秘籍中，它是`fr`分支。换句话说，命令`$ git checkout --ours numbers.txt`将在工作目录中产生如[列表 9-2](#list2) 所示的文件，而命令`$ git checkout --theirs numbers.txt`—如[列表 9-3](#list3) 所示的文件。请注意，这些命令不能解决冲突。它们只恢复文件的内容，而不改变其状态。恢复的文件保持在`UU`状态。
